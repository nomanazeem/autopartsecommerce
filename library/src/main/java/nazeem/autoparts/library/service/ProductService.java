...
public class ProductService {
    ...
    public Page<Product> findAllByCategoryId(Long categoryId, Pageable pageable) {
        ...
        Page<Product> pageProducts = new PageImpl<>(list, PageRequest.of(currentPage, pageSize), products.size());
    ...
    public void save(Product product) {
        ...
        if(product.getImage_posted1().getSize() > 0 ||
            ...
            if(product.getImage_posted1().getSize() > 0 ) {
                String image1 = ImageUpload(product.getImage_posted1());
                product.setImage1(image1);
            }

            if(product.getImage_posted2().getSize() > 0 ) {
                String image2 = ImageUpload(product.getImage_posted2());
                product.setImage2(image2);
            }

            if(product.getImage_posted3().getSize() > 0 ) {
                String image3 = ImageUpload(product.getImage_posted3());
                product.setImage3(image3);
            }

            if(product.getImage_posted4().getSize() > 0 ) {
                String image4 = ImageUpload(product.getImage_posted4());
    ...
    private String ImageUpload(MultipartFile productImage){
        String fileName = "";
